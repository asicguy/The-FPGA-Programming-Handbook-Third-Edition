// rm_passthrough.sv
// ------------------------------------
// Reconfigurable module: the identity function, and the socket contract around it
// ------------------------------------
// Author : Frank Bruno
//
...
// What this one computes is in rm_passthrough_core.sv.
//
...
module rm_passthrough
  ...
  (
   ...
   // No MAX_WIDTH. The windowed RMs need it to size their line buffers; a
   // point operation has no line buffers and so no width limit at all, and
   // carrying the parameter anyway would advertise a constraint that does not
   // exist. Parameters are not part of the socket's physical boundary -- only
   // the port list is -- so they may differ between RMs where the hardware
   // genuinely differs.
  ...
  localparam [31:0] KERNEL_ID = 32'hA5A50000;
