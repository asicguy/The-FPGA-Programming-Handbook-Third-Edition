...
# every instance name are theirs; what CH13 adds is this comment block, and in
...
# the sensor driver, which is now CH13's own Python (sw/ov5647.py) instead of
...
# `v_proc_sys` and `pixel_pack` by name -- CH13's `Ov5647Camera` checks for
...
# in the low byte. That is why CH13's filter treats a pixel as B,G,R,A rather
# than R,G,B,A the way CH11 did -- see HLS/src/video_filter.hpp.

proc ch13_create_mipi_hier {parentCell nameHier {pack_vlnv "xilinx.com:hls:pixel_pack_2:1.0"}} {

    if {$parentCell eq "" || $nameHier eq ""} {
        error "ch13_create_mipi_hier: empty argument"
    }
    set parentObj [get_bd_cells $parentCell]
    if {$parentObj eq ""} {
        error "ch13_create_mipi_hier: no cell '$parentCell'"
    }
    set parentType [get_property TYPE $parentObj]
    if {$parentType ne "hier"} {
        error "ch13_create_mipi_hier: parent <$parentObj> has TYPE = <$parentType>, expected <hier>"
    }
